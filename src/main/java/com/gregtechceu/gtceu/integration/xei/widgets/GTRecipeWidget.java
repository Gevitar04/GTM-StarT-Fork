...
public class GTRecipeWidget extends WidgetGroup {
    ...
    private void setRecipeWidget() {
        ...
        for (var capability : recipe.inputs.entrySet()) {
            capability.getKey().addXEIInfo(group, xOffset, recipe, capability.getValue(), false, true, yOff, minTier, tier);
        }
        for (var capability : recipe.tickInputs.entrySet()) {
            capability.getKey().addXEIInfo(group, xOffset, recipe, capability.getValue(), true, true, yOff, minTier, tier);
        }
        for (var capability : recipe.outputs.entrySet()) {
            capability.getKey().addXEIInfo(group, xOffset, recipe, capability.getValue(), false, false, yOff, minTier, tier);
        }
        for (var capability : recipe.tickOutputs.entrySet()) {
            capability.getKey().addXEIInfo(group, xOffset, recipe, capability.getValue(), true, false, yOff, minTier, tier);
        }
